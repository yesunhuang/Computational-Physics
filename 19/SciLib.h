...
/**
 * @name: quickSort(double *array,int left,int right)
 * @fuction: 快排
 * @param {arr}:排序数组
 * @param {left}:左终止
 * @param {right}:右终止
...
/**
 * @name: double* NumerovSolver(double (*f)(double x),double* initial,double* span,int step)
 * @fuction: Numerov法求解微分方程
 * @param {f}:f函数指针
 * @param {initial}:初始值{psi0,psi1}
 * @param {span}:求解范围 {span[0],span[1]}
 * @param {step}:步数
 * @return {double**}:psi(x),x in [span[0],span[1]]
 */
double** NumerovSolver(double (*f)(double x),double* initial,double* span,int step);

/**
 * @name: result[0][0]=span[0]; result[0][1]=initial[0];
 * @fuction: Numerov的辅助函数
 * @param {psix}:psi(x)
 * @param {fx}:f(x)
 * @param {h}:h
 * @return {double}:y(x)
