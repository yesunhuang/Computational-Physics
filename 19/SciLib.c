...
double** NumerovSolver(double (*f)(double x),double* initial,double* span,int step)
{
	int i;
	...
	for(i=0;i<2;i++)  
    	result[i]=(double*)malloc(sizeof(double)*(step+1));
	
	double h=(span[1]-span[0])/(double)step,y=0,sum=initial[1]*initial[1]*h;
	result[0][0]=span[0]; result[1][0]=initial[0];
	result[0][1]=span[0]+h; result[1][1]=initial[1];

	//差分求解
	for (i = 2; i < step+1; i++)
	{
		result[0][i]=result[0][i-1]+h;
		y=2*NumerovY(result[1][i-1],f(result[0][i-1]),h)
		   -NumerovY(result[1][i-2],f(result[0][i-2]),h)
		   +h*h*f(result[0][i-1])*result[1][i-1];
		result[1][i]=y/(1.0-h*h*f(result[0][i])/12.0);
		sum+=result[1][i]*result[1][i]*h;
	}
	...
	for ( i = 0; i < step+1; i++)
	{
		if	(sum>Epsilon)
		{
			result[1][i]=result[1][i]/sqrt(sum);
		}
	}
...
double NumerovY(double psix,double fx,double h)
{
	//返回y的值
	return (1.0-h*h*fx/12.0)*psix;
}
